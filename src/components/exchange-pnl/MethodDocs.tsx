...
import type { ExchangeId } from "@/lib/exchange-pnl/types";
...
export default function MethodDocs() {
  const [active, setActive] = useState<ExchangeId>("okx");
  const meta = getExchange(active);
  const doc = EXCHANGE_DOCS[active];
  const color = EXCHANGE_COLORS[active];
