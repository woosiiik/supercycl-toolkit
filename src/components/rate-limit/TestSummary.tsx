...
export default function TestSummary({ summary }: TestSummaryProps) {
  ...
  const hasRawDetails =
    (summary.rateLimitHeaders &&
      Object.keys(summary.rateLimitHeaders).length > 0) ||
    ...
    <div className="flex flex-col gap-3 rounded-md border border-zinc-300 p-4 dark:border-zinc-600">
      ...
      {summary.retryAfter != null && (
        <p className="text-sm text-zinc-700 dark:text-zinc-300">
          Retry-After:{" "}
          <span className="font-medium">{summary.retryAfter}초</span>
        </p>
      )}

      {summary.recoveryTimeMs != null && (
        <div className="rounded-md bg-green-50 p-3 text-sm text-green-700 dark:bg-green-900/20 dark:text-green-400">
          Rate-limit 해제까지 대기 시간:{" "}
          <span className="font-medium">
            {(summary.recoveryTimeMs / 1000).toFixed(1)}초
          </span>
